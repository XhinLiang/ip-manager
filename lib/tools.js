...
function getNewContent() {
    ...
    var content = getNewLine() + FLAG_START;
    for (var i = 0; i < hostslist.length; ++i) {
        var host = hostslist[i];
        for (var j = 0; j < host.ips.length; ++j) {
            var ip = host.ips[j];
            if (ip.active) {
                content += getNewLine() + ip.ip + " " + host.domain + " " + FLAG;
                continue;
            }
            content += getNewLine() + "#" + ip.ip + " " + host.domain + " " + FLAG;
        }
    }
    return content + getNewLine() + FLAG_END;
}

function readHostFile() {
    var promise = Q.Promise(function(resolve, reject) {
        var rl = Readline.createInterface({
            input: fs.createReadStream(getHostsFilePath(), {
                flags: 'r+'
            })
        });
